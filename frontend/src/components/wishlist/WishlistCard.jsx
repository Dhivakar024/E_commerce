...
import { ShoppingBag, Trash2, Eye, Star, FileText } from 'lucide-react';

export const WishlistCard = ({ product, index, onRemove, onMoveToBag }) => {
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-square w-full overflow-hidden bg-[#EFECE6] dark:bg-[#141E28] flex-shrink-0 select-none">
          ...
          {/* Top Actions: Remove from Wishlist & Quick View with 3D Pop */}
          <div className="absolute top-2 right-2 flex flex-col gap-1.5 z-10">
            <button
              onClick={() => onRemove(product.id)}
              className="w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200 backdrop-blur-md border bg-white/90 text-rose-600 border-black/10 hover:bg-rose-50 hover:scale-110 shadow-sm cursor-pointer"
          ...
          {/* Move to Bag Button (Desktop Slide-Up) */}
        ...
        <div className={`p-3 sm:p-3.5 flex flex-col flex-grow justify-between ${
          ...
          <div className="space-y-1">
            {/* Brand & Rating Row (Fixed Height: h-4) */}
          ...
          {/* Bottom Row (Pinned to bottom: mt-auto) */}
          <div className="mt-2.5 pt-2 border-t border-black/5 dark:border-white/10">
            ...
            <div className="mt-2 sm:hidden">
              <button
                onClick={() => onMoveToBag(product)}
                className="w-full py-1.5 bg-[#101820] hover:bg-[#C9A45C] text-[#F7F3EA] hover:text-[#101820] text-[9px] tracking-wider uppercase font-semibold flex items-center justify-center gap-1 transition-colors cursor-pointer"
              >
